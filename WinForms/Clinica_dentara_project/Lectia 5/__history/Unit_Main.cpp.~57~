...
void __fastcall TMain::SpeedButton15Click(TObject *Sender)
{
	...
	Memo2->Clear();
	 ...
	 TabSheet2->TabVisible=true;  // inregistare
...
void __fastcall TMain::SpeedButton23Click(TObject *Sender)
{
		...
		(Memo3->Text!="") && (Memo4->Text!="")  ) {
			...
			DM->Cartela_upd->Close();
			DM->Cartela_upd->ParamByName("MEDIC_FILIALA_ID")->AsInteger=MEDIC_ID;
			DM->Cartela_upd->ParamByName("CARTELA_ID")->AsInteger=DM->Qcartela->FieldByName("CARTELA_ID")->AsInteger;
			DM->Cartela_upd->ParamByName("DIAGNOZA")->AsString=Memo3->Text;
			DM->Cartela_upd->ParamByName("PRESCRIPTII")->AsString=Memo4->Text;
			DM->Cartela_upd->ParamByName("COST")->AsFloat=Edit17->Text.ToDouble();
			DM->Cartela_upd->ExecSQL();
			...
			TabSheet3->TabVisible=false; // editarea
			 TabSheet1->TabVisible=true; // main
...
void __fastcall TMain::SpeedButton16Click(TObject *Sender)
{
	if ( !DM->Qcartela->IsEmpty() ) {
		Edit16->Text=DM->Qcartela->FieldByName("MEDIC")->AsString;
		MEDIC_ID=DM->Qcartela->FieldByName("MEDIC_ID")->AsInteger;
		Memo3->Text=DM->Qcartela->FieldByName("DIAGNOZA")->AsString;
		Memo4->Text=DM->Qcartela->FieldByName("PRESCRIPTII")->AsString;
		Edit17->Text=DM->Qcartela->FieldByName("COST")->AsString;
		...
		MessageDlg("Nu aveti date de editat!", mtError, TMsgDlgButtons() << mbYes,0);
    }
...
void __fastcall TMain::SpeedButton4Click(TObject *Sender)
{
    ...
    Edit16->Text=DM->Qmedic->FieldByName("MEDIC")->AsString;
