...
{
	// atribuirea numarului cecului
	DM->QLiber->Close();
	DM->QLiber->SQL->Clear();
	DM->QLiber->SQL->Add(" UPDATE CEC SET NR_POZ=(SELECT COUNT(*) FROM VANZARE WHERE CEC_ID = :CEC_ID), NR_CEC=(SELECT ISNULL(MAX(NR_CEC), 0)+1 FROM CEC), SUMA_CEC =:SUMA_CEC WHERE CEC_ID =:CEC_ID ");
	DM->QLiber->ParamByName("CEC_ID")->AsInteger = CHECK_ID;
	DM->QLiber->ParamByName("SUMA_CEC")->AsFloat = Label10->Caption.ToDouble();
	DM->QLiber->ExecSQL();

	// facem resetare la tabela cu vanzari
	DM->QVanzare->Close();
	DM->QVanzare->ParamByName("CEC_ID")->AsInteger=CHECK_ID;
	DM->QVanzare->Open();

	// afisam suma totala a vanzarilor pe ziua de astazi
	DM->QLiber->Close();
	DM->QLiber->SQL->Clear();
	DM->QLiber->SQL->Add(" SELECT SUM(SUMA_TOTAL) AS ST FROM VANZARE V INNER JOIN CEC C ON C.CEC_ID = V.CEC_ID WHERE CAST(DATA_CEC AS DATE) = CONVERT(DATE, GETDATE())");
	DM->QLiber->Open();
	Label17->Caption = DM->QLiber->FieldByName("ST")->AsFloat;

    DM->frxCheck->ShowReport(true);
