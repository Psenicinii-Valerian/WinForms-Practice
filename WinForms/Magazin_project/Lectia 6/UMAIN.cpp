...
#pragma resource "*.dfm"
TFMAIN *FMAIN;
int CLIENT_ID=1, CHECK_ID, VANZARE_ID, COD_PRODUS=0;
float PROCENT=0;
//---------------------------------------------------------------------------
...
void __fastcall TFMAIN::SpeedButton5Click(TObject *Sender)
{
	...
	if(!DM->QIdentificare->IsEmpty()) //verificam daca primim ceva rezultate sau nu
	{
		if(!CBClient->Checked)//daca e produs
		{
			...
			if ( COD_PRODUS == 0 || COD_PRODUS != DM->QIdentificare->FieldByName("COD_PRODUS")->AsInteger ) {
				...
				DM->QLiber->Close();
				DM->QLiber->SQL->Clear();
				DM->QLiber->SQL->Add("SELECT MAX(VANZARE_ID) AS VANZARE_ID FROM VANZARE");
				DM->QLiber->Open();

				VANZARE_ID = DM->QLiber->FieldByName("VANZARE_ID")->AsInteger;
				COD_PRODUS = DM->QIdentificare->FieldByName("COD_PRODUS")->AsInteger;

				ShowMessage(VANZARE_ID);
				ShowMessage("IF");
		   }
					...
					DM->QLiber->ParamByName("VANZARE_ID")->AsInteger = VANZARE_ID;
